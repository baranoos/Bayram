...
function severityTone(value: string | null) {
  if (value?.toLowerCase().includes("ern")) {
    return "border-rose-200 bg-rose-50 text-rose-700";
  }

  if (value?.toLowerCase().includes("laag")) {
...
export default async function OverzichtPage({
  ...
  const gebreken = opdracht.resultaten.flatMap((resultaat) =>
    resultaat.gebreken.map((gebrek) => ({
      ...gebrek,
  ...
  for (const gebrek of gebreken) {
    ...
    if (!grouped.has(locatie)) {
      grouped.set(locatie, new Map());
    }

    const locationGroup = grouped.get(locatie)!;
    if (!locationGroup.has(categorie)) {
      locationGroup.set(categorie, []);
    }

    locationGroup.get(categorie)!.push(gebrek);
  }
        ...
        {gebreken.length === 0 ? (
          ...
          <div className="space-y-5">
            {Array.from(grouped.entries()).map(([locatie, categories]) => (
              <div key={locatie} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="mb-4 flex items-center justify-between gap-4">
                  <h3 className="text-lg font-semibold text-slate-950">{locatie}</h3>
                ...
                <div className="space-y-4">
                  {Array.from(categories.entries()).map(([categorie, items]) => (
                    <div key={categorie} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                      <div className="mb-3 flex items-center justify-between gap-4">
                        <h4 className="font-semibold text-slate-900">{categorie}</h4>
                      ...
                      <div className="grid gap-3 lg:grid-cols-2">
                        {items.map((gebrek) => (
                          <article key={gebrek.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <p className="font-medium text-slate-950">{gebrek.titel ?? "Gebrek"}</p>
                            ...
                            <p className="mt-2 text-sm text-slate-600">{gebrek.omschrijving ?? "Geen omschrijving geregistreerd."}</p>
