        ...
        {rapporten.length === 0 ? (
          ...
          <div className="space-y-3">
            {rapporten.map((rapport) => (
              <div key={rapport.id} className="flex flex-col gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-4 md:flex-row md:items-center md:justify-between dark:border-slate-700 dark:bg-slate-700/40">
                <div>
                  <p className="font-medium text-slate-900 dark:text-slate-100">{rapport.rapporttype}</p>
                  <p className="text-sm text-slate-500 dark:text-slate-400">{rapport.representatiefEmail ?? "Geen e-mail geregistreerd"}</p>
                </div>
                <span className="inline-flex w-fit rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-600 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300">
                  {rapport.status}
                </span>
              </div>
            ))}
